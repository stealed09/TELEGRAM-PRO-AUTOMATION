...
async def process_scrape(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🔍 Scraping member list...")
    result = await scraper.scrape_group_members(user_id, group, limit)

    if result['success']:
        csv_bytes, txt_bytes = scraper.build_output_files(result['members'], 'members')
...
async def scrape_active_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    try:
        await update.callback_query.edit_message_text(
            "🎯 **Scrape Active Users**\n\n"
            "Scans recent messages to find who has been active.\n\n"
            "✅ Skips admins & bots\n"
            "✅ Skips already-scraped users (global history)\n"
            "✅ Detects users with prior DM history → separate file\n\n"
            "Type: `<group_link> <message_limit>`\n"
            "Limit max: 20,000\n\n"
            "Example:\n`@mygroup 5000`",
            parse_mode='Markdown',
            reply_markup=menu_ui.back_button()
        ...
        pass
    context.user_data['awaiting_active_scrape'] = True

async def process_active_scrape(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.user_data.get('awaiting_active_scrape'):
        return
    ...
    group = parts[0]
    try:
        limit = min(int(parts[1]), 20000) if len(parts) > 1 else 1000
    except ValueError:
        limit = 1000
    context.user_data['awaiting_active_scrape'] = False
    ...
    result = await scraper.scrape_active_users(user_id, group, limit)
    ...
    history_users = result['history_users']

    await msg.edit_text(
        f"✅ **Scrape Complete!**\n\n"
        f"👥 Total active users: {result['total']}\n"
        f"🆕 New (not seen before): {result['new_count']}\n"
        f"💬 With chat history: {result['history_count']}\n\n"
        f"📁 Sending files...",
        parse_mode='Markdown'
    )

    context.user_data['last_scraped_users'] = all_users
    context.user_data['last_history_users'] = history_users

    if all_users:
        csv_bytes, txt_bytes = scraper.build_output_files(all_users, 'active')
        await update.message.reply_document(
            document=csv_bytes, filename="active_users.csv",
            caption=f"👥 All active users ({len(all_users)})"
        )
        await update.message.reply_document(document=txt_bytes, filename="active_users.txt")

    if history_users:
        hcsv, htxt = scraper.build_output_files(history_users, 'history')
        await update.message.reply_document(
            document=hcsv, filename="history_users.csv",
            caption=f"💬 Users with chat history ({len(history_users)})"
        )
        await update.message.reply_document(document=htxt, filename="history_users.txt")

    keyboard = [
        [InlineKeyboardButton("➕ Add All to Group", callback_data="add_to_group")],
        [InlineKeyboardButton("📨 Message All Users", callback_data="blast_message")],
        [InlineKeyboardButton("📨 Message History Users Only", callback_data="blast_history")],
        [InlineKeyboardButton("« Back to Scraper", callback_data="scraper")]
    ]
    await update.message.reply_text(
        "**What next?** Choose an action:",
        parse_mode='Markdown',
        reply_markup=InlineKeyboardMarkup(keyboard)
    )

# ──────────────────────────────────────────────
# NEW: Add Users to Group
# ──────────────────────────────────────────────

async def add_to_group_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    scraped = context.user_data.get('last_scraped_users', [])
    try:
        await update.callback_query.edit_message_text(
            f"➕ **Add Users to Group**\n\n"
            f"📋 {len(scraped)} users ready from last scrape\n\n"
            f"⚠️ Safety:\n"
            f"• ~18s delay between adds\n"
            f"• Auto-stops on PeerFlood\n"
            f"• Already-added users skipped forever\n\n"
            f"Type the target group:\n`@targetgroup` or invite link",
            parse_mode='Markdown',
            reply_markup=menu_ui.back_button()
        )
    except BadRequest:
    ...
    context.user_data['add_mode'] = 'scraped'

async def add_from_file_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    try:
        await update.callback_query.edit_message_text(
            "📁 **Add from File**\n\n"
            "Upload a CSV or TXT file with user IDs first.\n\n"
            "CSV: must have `id` column\n"
            "TXT: one user ID or @username per line",
            parse_mode='Markdown',
            reply_markup=menu_ui.back_button()
        )
    except BadRequest:
        pass
    context.user_data['awaiting_user_file'] = True
    context.user_data['file_action'] = 'add'

async def process_add_to_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.user_data.get('awaiting_add_group'):
        return

    user_id = update.effective_user.id
    target_group = update.message.text.strip()
    context.user_data['awaiting_add_group'] = False

    mode = context.user_data.get('add_mode', 'scraped')
    users = context.user_data.get('last_scraped_users', []) if mode == 'scraped' else context.user_data.get('file_users', [])

    if not users:
        await update.message.reply_text(
            "❌ No users to add. Scrape a group first or upload a file.",
            reply_markup=menu_ui.main_menu(is_admin=is_admin(user_id))
        )
        return
    ...
    msg = await update.message.reply_text(
        f"➕ Adding {len(user_ids)} users to `{target_group}`...\n⏳ This will take time.",
        parse_mode='Markdown'
    )
    ...
    async def status_update(added, failed, skipped, note=None):
        now = time.time()
        if now - last_update_time[0] < 10 and note is None:
            return
    ...
    result = await scraper.add_users_to_group(user_id, target_group, user_ids, status_update)

    if result['success']:
        flood_warn = "\n\n🚨 PeerFlood — stopped early." if result.get('peer_flood') else ""
        await msg.edit_text(
            f"✅ **Add Complete!**\n\n"
            f"✅ Added: {result['added']}\n"
            f"❌ Failed: {result['failed']}\n"
            f"⏭️ Skipped (already added): {result['skipped']}"
            f"{flood_warn}",
            parse_mode='Markdown',
            reply_markup=menu_ui.main_menu(is_admin=is_admin(user_id))
        )
    else:
        await msg.edit_text(f"❌ Error: {result['error']}", reply_markup=menu_ui.main_menu(is_admin=is_admin(user_id)))

# ──────────────────────────────────────────────
# NEW: Blast Message to Users
# ──────────────────────────────────────────────

async def blast_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    context.user_data['blast_target'] = 'all'
    users = context.user_data.get('last_scraped_users', [])
    try:
        await update.callback_query.edit_message_text(
            f"📨 **Message All Scraped Users**\n\n"
            f"👥 {len(users)} users targeted\n"
            f"⚠️ ~5s delay between messages\n\n"
            f"Type your message:",
            parse_mode='Markdown',
            reply_markup=menu_ui.back_button()
        )
        ...
        pass
    context.user_data['awaiting_blast_msg'] = True

async def blast_history_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    context.user_data['blast_target'] = 'history'
    users = context.user_data.get('last_history_users', [])
    try:
        await update.callback_query.edit_message_text(
            f"📨 **Message History Users**\n\n"
            f"💬 {len(users)} users with prior chat history\n"
            f"⚠️ ~5s delay between messages\n\n"
            f"Type your message:",
            parse_mode='Markdown',
            reply_markup=menu_ui.back_button()
...
async def blast_from_file_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    try:
        await update.callback_query.edit_message_text(
            "📁 **Message from File**\n\n"
            "Upload a CSV or TXT file with user IDs.\n"
            "I'll ask for your message after.",
            parse_mode='Markdown',
            reply_markup=menu_ui.back_button()
        )
    except BadRequest:
        pass
    context.user_data['awaiting_user_file'] = True
    context.user_data['file_action'] = 'blast'

async def process_blast_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.user_data.get('awaiting_blast_msg'):
        return

    user_id = update.effective_user.id
    message_text = update.message.text.strip()
    context.user_data['awaiting_blast_msg'] = False

    target = context.user_data.get('blast_target', 'all')
    if target == 'history':
        users = context.user_data.get('last_history_users', [])
        label = "history users"
    elif target == 'file':
        users = context.user_data.get('file_users', [])
        label = "file users"
    else:
        users = context.user_data.get('last_scraped_users', [])
        label = "all scraped users"

    if not users:
        await update.message.reply_text("❌ No users. Scrape first or upload a file.",
            reply_markup=menu_ui.main_menu(is_admin=is_admin(user_id)))
        return

    user_ids = [u['id'] for u in users if u.get('id')]
    msg = await update.message.reply_text(
        f"📨 Sending to {len(user_ids)} {label}...\n⏳ Please wait.", parse_mode='Markdown'
    )

    import time
    last_t = [0]

    async def status_cb(sent, failed, note):
        now = time.time()
        if now - last_t[0] < 8 and note is None:
            return
        last_t[0] = now
        try:
            await msg.edit_text(f"📨 **Sending...**\n\n✅ Sent: {sent}\n❌ Failed: {failed}", parse_mode='Markdown')
        except Exception:
            pass

    result = await scraper.blast_message_to_users(user_id, user_ids, message_text, status_cb)

    if result['success']:
        await msg.edit_text(
            f"✅ **Message Blast Done!**\n\n✅ Sent: {result['sent']}\n❌ Failed: {result['failed']}",
            parse_mode='Markdown',
            reply_markup=menu_ui.main_menu(is_admin=is_admin(user_id))
        )
    else:
...
async def handle_document_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await check_access(user_id):
        return
    if not context.user_data.get('awaiting_user_file'):
        return
    ...
    if not (filename.endswith('.csv') or filename.endswith('.txt')):
        await update.message.reply_text("❌ Please upload a `.csv` or `.txt` file.", parse_mode='Markdown')
        return
    ...
    try:
        ...
        users = scraper.parse_user_file(bytes(file_bytes), filename)
    except Exception as e:
    ...
    context.user_data['file_users'] = users
    context.user_data['awaiting_user_file'] = False
    file_action = context.user_data.get('file_action', 'add')

    if file_action == 'blast':
        context.user_data['blast_target'] = 'file'
        await update.message.reply_text(
            f"✅ Loaded **{len(users)}** users from file.\n\nType your message to send them:",
            parse_mode='Markdown', reply_markup=menu_ui.back_button()
        )
        context.user_data['awaiting_blast_msg'] = True
    else:
        await update.message.reply_text(
            f"✅ Loaded **{len(users)}** users from file.\n\n"
            f"Now type the target group:\n`@targetgroup`",
            parse_mode='Markdown', reply_markup=menu_ui.back_button()
        )
        context.user_data['awaiting_add_group'] = True
        context.user_data['add_mode'] = 'file'
...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif context.user_data.get('awaiting_active_scrape'):
        await process_active_scrape(update, context)
    elif context.user_data.get('awaiting_add_group'):
        await process_add_to_group(update, context)
    elif context.user_data.get('awaiting_blast_msg'):
        await process_blast_message(update, context)
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(scrape_active_handler, pattern='^scrape_active$'))
    application.add_handler(CallbackQueryHandler(add_to_group_handler, pattern='^add_to_group$'))
    application.add_handler(CallbackQueryHandler(add_from_file_handler, pattern='^add_from_file$'))
    application.add_handler(CallbackQueryHandler(blast_message_handler, pattern='^blast_message$'))
    application.add_handler(CallbackQueryHandler(blast_history_handler, pattern='^blast_history$'))
    application.add_handler(CallbackQueryHandler(blast_from_file_handler, pattern='^blast_from_file$'))
    ...
    # Document upload handler (for CSV/TXT user files)
