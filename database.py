...
class Database:
    ...
    async def init_db(self):
        async with aiosqlite.connect(self.db_name) as db:
            ...
            # Scraped users history (global, per bot-user) — NEW
            await db.execute('''
                CREATE TABLE IF NOT EXISTS scraped_users_history (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    bot_user_id INTEGER,
                    tg_user_id INTEGER,
                    username TEXT,
                    ...
                    UNIQUE(bot_user_id, tg_user_id)
                )
            ''')

            # Added-to-group history — NEW
            await db.execute('''
                CREATE TABLE IF NOT EXISTS added_users_history (
                    ...
                    UNIQUE(bot_user_id, group_id, tg_user_id)
                )
            ''')
    ...
    # ============ SCRAPED USERS HISTORY (NEW) ============

    async def get_all_scraped_user_ids(self, bot_user_id):
        async with aiosqlite.connect(self.db_name) as db:
            async with db.execute(
                'SELECT tg_user_id FROM scraped_users_history WHERE bot_user_id = ?',
                (bot_user_id,)
            ) as cursor:
                rows = await cursor.fetchall()
                return [r[0] for r in rows]

    async def save_scraped_users(self, bot_user_id, users: list):
        async with aiosqlite.connect(self.db_name) as db:
            for u in users:
                try:
                    await db.execute(
                        '''INSERT OR IGNORE INTO scraped_users_history
                           (bot_user_id, tg_user_id, username, first_name, last_name)
                           VALUES (?, ?, ?, ?, ?)''',
                        (bot_user_id, u['id'], u.get('username'), u.get('first_name'), u.get('last_name'))
                    )
                except Exception:
                    pass
            await db.commit()
    ...
    async def get_added_user_ids(self, bot_user_id, group_id):
        async with aiosqlite.connect(self.db_name) as db:
            async with db.execute(
                'SELECT tg_user_id FROM added_users_history WHERE bot_user_id = ? AND group_id = ?',
                (bot_user_id, str(group_id))
            ) as cursor:
                rows = await cursor.fetchall()
                return [r[0] for r in rows]

    async def save_added_user(self, bot_user_id, group_id, tg_user_id):
        async with aiosqlite.connect(self.db_name) as db:
            try:
                await db.execute(
                    '''INSERT OR IGNORE INTO added_users_history
                       (bot_user_id, group_id, tg_user_id)
                       VALUES (?, ?, ?)''',
                    (bot_user_id, str(group_id), tg_user_id)
                )
                await db.commit()
            except Exception:
                pass
