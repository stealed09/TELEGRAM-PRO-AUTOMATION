...
class Scraper:
    ...
    # ─────────────────────────────────────────────
    # NEW: Add users to a group (with flood/ban protection)
    # ─────────────────────────────────────────────
    async def add_users_to_group(self, user_id, target_group, user_ids_to_add,
                                  status_callback=None):
        """
        Add users to a group one by one with:
        - Flood wait handling (auto-sleep)
        - PeerFlood detection (stop & report)
        - Skip already-members
        - Skip already-added (DB history)
        - Delay between adds to avoid ban
        Returns dict with added/failed/skipped counts
        """
        try:
            account = await db.get_active_account(user_id)
            if not account:
